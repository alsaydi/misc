...
int count_intervals(int * arr, int len, node * list){
	...
	*/
	while (temp){		
		...
		smaller_seg = find_segments_with_smaller_min(arr, list->next, temp);
		if (smaller_seg)
		{
			seg = smaller_seg->next;
		}
		fprintf(stdout, "%s : ", "Current node");
		print_node(temp,arr);
		print_segments(arr, len, smaller_seg);
		min_so_far = len;
		while (NULL != seg){
			if (arr[seg->start] > min_so_far)
			{
				seg = seg->next;
				continue;
			}
			...
			{
				min_so_far = arr[seg->start];
			}
			if (max > arr[seg->end] /* if max is not greater than segment's end then we cannot use the current temp as and end for a number starting from segment*/
				&& min > arr[seg->start]){
				if (arr[seg->end] < min){ /* the current segment's max is less than the min, take every number is segment*/
					prev_seg_nums = seg->end - seg->start + 1;
				}
				else{
					curr_min_idx = approximate_binary_search(arr, seg->start, seg->end, arr[temp->start]);
					prev_seg_nums = curr_min_idx - seg->start;
				}
				fprintf(stdout, "curr min %d found at prev seg at %d\n", arr[temp->start], curr_min_idx);
				prev_max_idx = approximate_binary_search(arr, temp->start, temp->end, max_so_far);				
				curr_seg_nums = temp->end - prev_max_idx;
				if (max_so_far < arr[prev_max_idx]){
					curr_seg_nums++;
				}
				//if(mult == 0) mult = 1;
